...
//! count of the selection. Read selection state via `handle.signal()`.
//!
...
fn App() -> Element {
    ...
    // Subscribe to the underlying signal to render a live selection count.
    let selected_count = table.signal().read().selection.len();
