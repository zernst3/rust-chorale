...
- **Nested-grouping expand/collapse no longer drops the parent header (#36).** With two-level grouping in the default `DataRowsOnly` pagination mode, expanding a depth-0 group whose children stayed collapsed made the parent header vanish — its (collapsed) child headers took its slot — and it only reappeared once a grandchild was expanded. Root cause is in `chorale-core`: `paginate_grouped` kept an *expanded* header only when its subtree contained a data row on the current page. An expanded parent with all children collapsed has no visible data rows in its subtree, so it was dropped, while its collapsed child headers (which always render) stayed. An expanded header is now also kept when its subtree contains a collapsed descendant header — i.e. whenever any descendant will render — so ancestors of any visible row always render. Because the bug was in shared core, **both adapters were affected and both are fixed** (an earlier diagnosis that pinned it on Dioxus keying and called Leptos unaffected was wrong). Group headers still key on their `GroupKey` and data rows on their `RowId` for stable identity. Verified in both the Dioxus and Leptos QA harnesses plus a core regression test; the combined `examples/nested-collapse-qa` harness exercises it.

### Added

- **Expand-all / collapse-all group toggle.** New opt-in `group_expand_toggle` prop on `<Table>` (both adapters). When the table is grouped, a chevron control (`▶` collapsed / `▼` expanded, a step larger than the per-row group arrows and aligned directly above them) appears at the left of the first column header: clicking it expands every group at all depths, and clicking again collapses them all. The icon and its `title` / `aria-label` reflect the current state, and the click stops propagation so it never triggers that column's sort. Off by default and a no-op when the table is not grouped; backed by the existing `expand_all_groups` / `collapse_all_groups` transitions. Dioxus + Leptos.
