    ...
    TRow: Clone + PartialEq + Send + Sync + 'static,
{
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        // Holds the pending debounced-measurement timeout so a newer trigger
        // can cancel it (see the debounce comment in the Effect below).
        let meas_debounce: StoredValue<Option<TimeoutHandle>> = StoredValue::new(None);
        Effect::new(move |_| {
            ...
            // Debounce the DOM measurement. Two reasons:
            //
            // 1. Post-patch timing. In Leptos 0.8 a user `Effect` and the
            //    view's `RenderEffect`s are ALL channel-driven `spawn_local`
            //    tasks with no relative ordering guarantee, and this Effect is
            //    created before the view, so on a trigger change it can wake
            //    FIRST and measure the PRE-PATCH DOM (stale heights, no diff,
            //    index-shifted cache survives → rows vanish after a collapse,
            //    scroll jump when a detail panel is measured late). Firing on a
            //    timeout runs after the render task patched the DOM, so
            //    getBoundingClientRect returns real post-patch heights.
            //
            // 2. Momentum. A per-frame measurement (rAF) runs the
            //    scroll-anchoring `set_scroll_top` below on EVERY inertial
            //    scroll frame, and each programmatic scrollTop write cancels
            //    the browser's momentum — felt as a "bounce" at the bottom on
            //    a trackpad. Coalescing rapid scroll-bucket triggers into one
            //    measurement after scrolling settles keeps the anchoring write
            //    off the momentum path (the same effect the Dioxus adapter
            //    gets for free from its slower async `document::eval`).
            meas_debounce.update_value(|h| {
            ...
            let handle = set_timeout_with_handle(
                move || {
                    ...
                    let Ok(node_list) = container
                        .query_selector_all(":scope > table > tbody > tr[data-chorale-index]")
                    else {
                        return;
                    };
            ...
            .ok();
            meas_debounce.set_value(handle);
        });
    }
