...
/// The primary chorale table component.
///
/// `sort_enabled` (default `true`) controls whether sortable column headers
/// are clickable and show a direction arrow. Set to `false` to hide all sort
/// affordances without clearing existing sort state.
///
/// Scroll events are not yet wired (item 12). The visible window is computed
/// from `scroll_top` in state (defaults to 0.0).
/// Props for the [`Table`] component (generated by `#[component]`).
///
/// | Prop | Default | Description |
/// |---|---|---|
/// | `handle` | — | Reactive table handle from `use_table`. |
/// | `sort_enabled` | `true` | Show sort arrows and make sortable headers clickable. |
/// | `filter_enabled` | `false` | Show a filter input row below the column headers. |
#[component]
